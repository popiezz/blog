var PZ_REPO = { owner: "Filip", repo: "blog", branch: "main" };
...
function pzMarkdownToHtml(md) {
  ...
  lines.forEach(function (line) {
    ...
    if (inCode) {
      codeBuf.push(pzEscapeHtml(line));
      return;
    }
    ...
    if (h) {
      ...
      flushList();
      html +=
        "<h" + h[1].length + ">" + pzInline(h[2]) + "</h" + h[1].length + ">\n";
      return;
    }
    ...
    if (bq) {
      flushPara();
      flushList();
      html += "<blockquote>" + pzInline(bq[1]) + "</blockquote>\n";
      return;
    }
...
function pzPostFromRaw(slug, raw) {
  ...
  return {
    ...
    date: meta.date || "",
    tags: pzTags(meta).map(function (label) {
      return { label: label, cls: pzTagClass(label) };
    }),
...
async function pzFetchPosts() {
  ...
  if (!listRes.ok)
    throw new Error("Could not list posts (" + listRes.status + ")");
  ...
  posts.sort(function (a, b) {
    return pzDateValue(b.date) - pzDateValue(a.date);
  });
  return posts;
}
